            ...
            if os.path.exists(os.path.join(path, filename)):
                ...
                if len(files) == 0:
                    ...
                    for f in files:
                        ...
                        no: int = int(basname_wo_ext[-digits:])
                        if no >= begin:
                            sno_list.append(int(basname_wo_ext[-digits:]))

                    if len(sno_list) == 1:
                        # 現在のシーケンス番号のみ -> +1
                        _debug_print(f'Last Sequencial No. is {begin}.')
                    else:
                        sno: int = begin
                        for x, y in zip(sno_list, sno_list[1:]):
                            if x != y and x >= begin:
                                sno = x + 1
                                break
                        else:
                            sno = sno_list[len(sno_list) - 1]
                            _debug_print(f'Last Sequencial No. is {sno}.')
